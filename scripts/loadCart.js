...
const changeQuantity = () => {
  ...
  quantityInput.forEach((input) => {
    input.addEventListener('input', (e) => {
      ...
      const currentName = currentProduct.getElementsByClassName('pname')[0]
        .textContent;
...
const deleteElement = () => {
  ...
  deleteBtn.forEach((button) => {
    button.addEventListener('click', (e) => {
      e.preventDefault();
      const removedElement = e.target.parentNode.parentNode.getElementsByClassName(
        'pname'
      )[0].textContent;
      ...
      cartList.some((product) => {
        if (product.name === removedElement)
          cartList.forEach((element, index) => {
            if (element.name === removedElement) cartList.splice(index, 1);
          });
      });
      localStorage.setItem('list', JSON.stringify(cartList));
      addItemsToCart();
    ...
    cartElements.innerHTML += `
        <div class="product-item">
            ...
            <div class="item-content">
                <div>
                    <span class="pprice">total: ${
                      element.price * element.quantity
                    }</span>
                    <input type="number" name="quantity" min="1" max="20" value="${
                      element.quantity
                    }" class="quantity-input" onkeydown="return false"></input>
                </div>
