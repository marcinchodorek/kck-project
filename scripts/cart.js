...
const displayProduct = (data, sortType = '') => {
    sortType != '' ?
        sortType === 'highest' ? data.sort((a, b) => b.price - a.price) :
            (sortType === 'lowest' ? data.sort((a, b) => a.price - b.price) :
                (sortType === 'asc' ? data.sort((a, b) => (a.name > b.name) ? 1 : ((b.name > a.name) ? -1 : 0)) :
    ...
    data.forEach(product => {
        ...
        <div class="product-item">
            ...
            <div class="item-content">
                ...
                <div><span class="pprice">${product.price}</span>zł</div>
...
const nameSearch = (search) => {
    ...
    Array.from(productName).filter(item => !item.textContent.toLowerCase().includes(search)).forEach(item => item.parentNode.parentNode.classList.add('filtered'));
    Array.from(productName).filter(item => item.textContent.toLowerCase().includes(search)).forEach(item => item.parentNode.parentNode.classList.remove('filtered'));
}
